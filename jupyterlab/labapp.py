...
class LabApp(NBClassicConfigShimMixin, LabServerApp):
    ...
    * App mode: JupyterLab allows multiple JupyterLab "applications" to be
    ...
    flags['mirror-sync'] = (
        {'LabApp': {'mirror_sync': True}},
        "Sync notebook dir to minio buckets (experimental)."
    )
    ...
    default_url = Unicode('/doc', config=True,
    ...
    mirror_sync = Bool(False, config=True, help="Sync notebook dir to minio buckets (experimental).")
    ...
    def initialize_templates(self):
        ...
        if self.dev_mode:
            ...
            self.log.info('Static paths: {}'.format(self.static_paths))
            self.log.info('Template paths: {}'.format(self.template_paths))
    ...
    def initialize_handlers(self):
        ...
        if 'hub_prefix' in self.serverapp.tornado_settings:
            ...
            if self.mirror_sync:
                # Only idg-notebook image can take effect
                source_dir = "/data/notebooks/default"
                target_dir = "myminio/notebooks/{}/default".format(page_config['hubUser'])
                cmd = 'echo "*/2 * * * * root mc mirror {} {} --overwrite" >> /etc/crontab'.format(source_dir, target_dir)
                print(cmd)
                os.system(cmd)
                os.system("service cron start")
                self.log.info("Sync notebook dir to minio buckets")
        ...
        # self.log.info('App settings: {}'.format(self.serverapp.web_app.settings))
